...
BattleBuffMgr.GetBuffCountByCardAndType = function(card, type, ...)
...
BattleBuffMgr.GetBuffListByCardAndEffectID = function(card, effectID, ...)
...
BattleBuffMgr.GetCardIsContainEffect = function(card, effectID, ...)
  -- function num : 0_16 , upvalues : self, ipairs, _ENV
  if card == nil then
    return false
  end
  local buffList = (self.GetBuffList)()
...
BattleBuffMgr.GetShieldInfo = function(posIndex, ...)
  -- function num : 0_19 , upvalues : self, ipairs
  local buffList = (self.GetBuffList)()
  for _,v in ipairs(buffList) do
...
BattleBuffMgr.DealClearBuff = function(targetCard, newBuff, ...)
  -- function num : 0_21 , upvalues : _ENV, ipairs, t_insert
...
BattleBuffMgr.CleanBuffGroup = function(targetCard, buffGroup, newBuff, ...)
  -- function num : 0_22 , upvalues : split, _ENV, tonumber, ipairs
  local isClear = false
  local clearCount = 0
  local split = split
        ...
        for _,v in ipairs(clearBuffTable) do
          ...
          if type == 1 then
            ...
            if type == 2 then
              ...
              buffTable = (BattleBuffMgr.GetBuffListByCardAndEffectID)(targetCard, value)
            end
          end
...
BattleBuffMgr.SetAllBuffRoundActive = function(state, ...)
  -- function num : 0_23 , upvalues : _ENV, ipairs
