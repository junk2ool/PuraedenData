...
, 
BackgroundRect = {}
, DefaultGridCount = 0, GridLength = 0, GridProject = Vector2.zero, XAxis = Vector2.zero, YAxis = Vector2.zero, ZAxis = Vector2.zero, ZAxisYScale = 1, NormalizeXAxis = Vector2.zero, NormalizeYAxis = Vector2.zero, DefaultFloor = 0, DefaultWall = 0, DefaultBackGround = 0, DefaultZoom = 0, MaxZoom = 0, MinZoom = 0, MaxChooseCircleSize = 0, RoleMaxMoveDistance = 5, RoleMaxWaitDuration = 5, RoleMinWaitDuration = 0, RoleScale = 1, RandomBubbleGap = 10}
...
HomelandFurnitureType = {All = 0, Wall = 1, Floor = 2, Furniture = 3, Decorate = 4, Carpet = 5, Background = 6}
...
HomelandData.InitRoomConstantPart1 = function(...)
  ...
  HomelandRoomConstant.DefaultBackGround = tonumber(info[3])
...
roomStyle = {}
          ...
          if config.type == HomelandFurnitureType.Wall then
            ...
            if config.type == HomelandFurnitureType.Floor then
              ...
              if config.type == HomelandFurnitureType.Background then
                ((HomelandData.StyleWF)[styleData.styleId]).Background = {Id = subData.id, Uid = subData.uid}
              end
            end
          end
    ...
    ;
    (HomelandData.InitRoomStyleInfo)(defaultStyleData, ((HomelandData.StyleWF)[styleData.styleId]).Wall, ((HomelandData.StyleWF)[styleData.styleId]).Floor, ((HomelandData.StyleWF)[styleData.styleId]).Background)
...
HomelandData.InitRoomStyleInfo = function(data, wall, floor, background, ...)
  ...
  if not wall or not floor or not background then
    ...
    for i = 1, count do
      ...
      if config.type == HomelandFurnitureType.Wall and not wall then
        ...
        if config.type == HomelandFurnitureType.Floor and not floor then
          ...
          if config.type == HomelandFurnitureType.Background and not background then
            _roomData.Background = {Id = subData.id, Uid = subData.uid}
          end
        end
      end
    end
...
HomelandData.SetBackgroundArea = function(_angle, _pos, ...)
  ...
  HomelandRoomConstant.BackgroundRect = {
TopLeft = {x = pos.x, y = pos.y}
, 
TopRight = {x = pos.x + cos, y = pos.y + sin}
, 
...
HomelandData.UpdateCardGridUsage = function(x, y, id, ...)
  -- function num : 0_18 , upvalues : _roomData, _ENV
...
HomelandData.CheckGridAvailableForRole = function(x, y, ...)
  -- function num : 0_19 , upvalues : _roomData
...
HomelandData.GetRandomAvailableCoordinate = function(...)
  -- function num : 0_21 , upvalues : _ENV, Random
...
HomelandData.GetPath = function(start, destination, ...)
  ...
  while 1 do
    ...
    -- function num : 0_26_0 , upvalues : closeSet, minNode, _ENV, destination, openSet, nearBySet, checkNode
    ...
    -- function num : 0_26_1 , upvalues : checkNode, _ENV, destination, node, closeSet, addNewNode
