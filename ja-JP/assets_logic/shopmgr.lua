...
, 
TuiSongLiBao = {btnName = "TuiSongLiBao", tapType = 3, type = 6}
, 
...
ShopMgr.SetProductData = function(data, ...)
  ...
  if not data then
    ...
    if isOpen then
      UIMgr:SendWindowMessage((WinResConfig.HomeWindow).name, (WindowMsgEnum.HomeWindow).E_MSG_REFRESH_PRODUCT, data)
    end
  end
end
